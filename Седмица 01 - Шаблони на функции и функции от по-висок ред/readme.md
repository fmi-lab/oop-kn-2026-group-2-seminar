# Седмица 01 - Шаблони на функции и функции от по-висок ред
...
**ВАЖНО**: Не използвайте `auto` никъде другаде, освен при работа с анонимни функции. Освен, че е лош стил да оставяме компилаторът да си изведе типовете, в някои случаи той може да не успее и да получим неочаквани грешки по време на изпълнение.

## Инструкции:
За всяка от задачите, в която се казва функцията да приема масив, се има предвид да се приема масив като `range` - при това представяне подаваме масива като два указателя - първият указател сочи първия елемент на масива, а втория - след последния елемент на масива. Използвайки функциите `std::begin` и `std::end` лесно можем да намерим тези указатели за всеки масив. В стандартната библиотека на езика, всички функции над масиви приемат `range` представяне.
...
```c++
int arr[] = {1, 2, 3, 4};
print(std::begin(arr), std::end(arr)); // -> 1 2 3 4
```
