...
Методите `get_numerator` и `get_denominator` се наричат селектори - чрез тях достъпваме член-данните за "четене" (т.е. не можем да ги модифицираме). Методите `set_numerator` и `set_denominator` се наричат мутатори - чрез тях променяме член-данните. Обърнете внимание на проверката за 0 във `set_denominator` - тя ни гарантира, че инвариантът е винаги спазен, т.е. няма как знаменателят да стане равен на 0. Свойството на обектите да ограничават директния достъп до член-данните си, за да бъде спазен някакъв инвариант, се нарича енкапсулация и това е един от основните принципи на ООП.
